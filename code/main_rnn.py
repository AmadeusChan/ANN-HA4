...
tf.app.flags.DEFINE_string("RNN_type", "BasicRNN", "Training directory.")
...
tf.app.flags.DEFINE_float("weight_decay",   0., "L2 loss")
		...
		for wd in [0.]:
			for kb in [1.]:
				...
				if not ((abs(lr - 3e-5) < 1e-10 and abs(wd - 3e-5) < 1e-10 and abs(kb - .5) < 1e-10) or (abs(lr - 6e-6) < 1e-10 and abs(wd - 1e-5) < 1e-10 and abs(kb - .7) < 1e-10)):
					continue
                                '''
				hyparam_str = "BasicRNN__learning_rate_" + str(lr) + "__weight_decay_" + str(wd) + "___keep_prob_" + str(kb)
