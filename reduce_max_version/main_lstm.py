		...
		for wd in [1e-8]:
			for kb in [.85]:
				...
				if not ((abs(lr - 3e-5) < 1e-10 and abs(wd - 3e-5) < 1e-10 and abs(kb - .5) < 1e-10) or (abs(lr - 6e-6) < 1e-10 and abs(wd - 1e-5) < 1e-10 and abs(kb - .7) < 1e-10)):
					continue
                                '''
				hyparam_str = "final_network"
				#hyparam_str = "_otest_1_inner_dropout_max_1mlp__1layer_LSTM__learning_rate_" + str(lr) + "__weight_decay_" + str(wd) + "___keep_prob_" + str(kb)
				with tf.variable_scope("test_lr"):
					...
					t_acc = 0.
					    ...
					    if flag:
			        	    ...
			        	    print("        max validation_accuracy [%.8f],  test_ accuracy [%.8f]" % (max_dev_acc, t_acc))
