...
class RNN(object):
	...
	with tf.variable_scope("foward_cell"):
		...
		if RNN_type == "LSTM":
			fw_cell = BasicLSTMCell(num_units, keep_prob = self.keep_prob)
		else:
	...
	with tf.variable_scope("barkward_cell"):
		...
		if RNN_type == "LSTM":
			bw_cell = BasicLSTMCell(num_units, keep_prob = self.keep_prob)
		else:
	...
	outputs, states = tf.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell, tf.nn.dropout(self.embed_input, keep_prob=self.keep_prob), self.texts_length, dtype = tf.float32, scope = "bi_lstm")
	'''

	with tf.variable_scope("foward_cell2"):
		if RNN_type == "LSTM":
			fw_cell = BasicLSTMCell(num_units)
		else:
			fw_cell = GRUCell(num_units)
	with tf.variable_scope("barkward_cell2"):
		if RNN_type == "LSTM":
			bw_cell = BasicLSTMCell(num_units)
		else:
			bw_cell = GRUCell(num_units)

	outputs, states = tf.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell, tf.concat((self.embed_input, outputs[0] + outputs[1]), axis = 2), self.texts_length, dtype = tf.float32, scope = "bi_lstm2")
	'''
	...
	#self.y1 = tf.layers.dense(inputs = self.y0_dp, units = 128, activation = tf.nn.relu)
    ...
    def train_step(self, session, data, summary=False):
        ...
        output_feed = [self.loss, self.accuracy, self.gradient_norm, self.train_op]
        if summary:
