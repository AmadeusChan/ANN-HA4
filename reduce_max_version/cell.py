...
class BasicLSTMCell(tf.contrib.rnn.RNNCell):
    ...
    def __init__(self, num_units, forget_bias=1.0, activation=tf.tanh, reuse=None, keep_prob = 1.):
        self._num_units = num_units
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or "basic_lstm_cell", reuse=self._reuse):
            c, h = state
	    #c = tf.nn.dropout(c, keep_prob = self._keep_prob)
	    #h = tf.nn.dropout(h, keep_prob = self._keep_prob)
